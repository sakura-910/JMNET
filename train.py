...
def train(train_loader, model, optimizer, epoch, start_iter, cfg):
    ...
    for iter, data in enumerate(train_loader):
        ...
        # #pred
        # loss_pred = torch.mean(outputs['loss_pred'])
        # losses_pred.update(loss_emb.item())
        ...
        if iter % 20 == 3:
            output_log = '({batch}/{size}) LR: {lr:.6f} | Batch: {bt:.3f}s | Total: {total:.0f}min | ' \
                         ...
                         'Loss(text/kernel/emb/pred): {loss_text:.3f}/{loss_kernel:.3f}/{loss_emb:.3f}/{loss_rec:.3f} ' \
                         '| IoU(text/kernel): {iou_text:.3f}/{iou_kernel:.3f}'.format(     #| Acc rec: {acc_rec:.3f}'
                batch=iter + 1,
...
#def adjust_learning_rate(optimizer, epoch, cfg):
    schedule = cfg.train_cfg.schedule   #polylr
...
def save_checkpoint(state, checkpoint_path, cfg):
    ...
    if cfg.data.train.type in ['synth'] or \
            (state['iter'] == 0 and state['epoch'] > cfg.train_cfg.epoch - 300 and state['epoch'] % 10 == 0):
        file_name = 'checkpoint_%dep_res18.pth.tar' % (state['epoch'])
