...
class UnESPLoss_v2(nn.Module):
    def __init__(self, feature_dim=4, loss_weight=1.0):   #4->8
        super(UnESPLoss_v2, self).__init__()
    ...
    def forward_single(self, emb, instance, kernel, training_mask, bboxes):
        ...
        for i, lb in enumerate(unique_labels):
            ...
            dist = F.relu(dist) ** 2
            l_agg[i] = torch.mean(dist)
        l_agg = torch.sum(l_agg[1:])
        # l_agg = l_agg[1:]
        # l_agg = torch.sum(l_agg[1:])
