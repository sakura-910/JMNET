...
class Cluster_Head(nn.Module):
    def __init__(self,
        ...
        # self.pred_loss = build_loss(loss_pred)
       ...
       # print(label)
        ...
        for i in range(1, label_num):
            ...
            if cfg.test_cfg.bbox_type == 'rect':
                ...
                binary[ind] = 1
                contours, img = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                bbox = contours[0] * scale
               # cv2.polylines(binary, contours, -1, 255, 6)
               # cv2.drawContours(binary, contours, -1, (255, 255, 0), 6)
           ...
           #  img = cv2.drawContours(label, contours, -1, (255, 0, 0), 4) #
           #  plt.imshow(img)
           #  plt.show()
    ...
    def loss(self, out, gt_texts, gt_kernels, training_masks, gt_instances, gt_bboxes):
        ...
        #loss_text = self.text_loss(texts, gt_texts, training_masks, reduce=False)
        #loss_text = self.text_loss(texts, gt_texts, reduce=False)
        ...
        for i in range(kernels.size(1)):
            kernel_i = kernels[:, i, :, :]   #pred kernels
            gt_kernel_i = gt_kernels[:, i, :, :]
            #pred

            loss_kernel_i = self.kernel_loss(kernel_i, gt_kernel_i, selected_masks, reduce=False)
            #loss_kernel_i = self.kernel_loss(kernel_i, gt_kernel_i, reduce=False)
        ...
        # iou_kernel = iou(
        #     (kernels[:, -1, :, :] > 0).long(), gt_kernels[:, -1, :, :], reduce=False)
        losses.update(dict(
        ...
        # embedding loss
        loss_emb = self.emb_loss(embs, gt_instances, gt_kernels[:, -1, :, :], selected_masks, gt_bboxes, reduce=False)
        losses.update(dict(
            loss_emb=loss_emb
        ))
        ...
        #     texts_i = texts[:, i, :, :]
        #
        # loss_pred = self.pred_loss(embs, texts_i, kernel_i, training_masks, gt_bboxes, reduce=False)
        # losses.update((dict(
        #     loss_pred=loss_pred
        # )))
