...
def box_center(points):
    ...
    return center_x, center_y, x1, y1, x2, y2, x3, y3, x4, y4
...
def sorted_boxes(boxes):
    #sorted by the left top point's x loctiion
...
def create_affine_boxes(boxes):
    ...
    for boxes_1, boxes_2 in zip(boxes[:-1],boxes[1:]):
        center_x1, center_y1, x1, y1, x2, y2, x3, y3, x4, y4 = box_center(boxes_1)
        ...
        center_x2, center_y2, x1, y1, x2, y2, x3, y3, x4, y4 = box_center(boxes_2)
...
if __name__ == "__main__":
    ...
    pts = np.float32([[a_boxes[0][0], a_boxes[0][1]], [a_boxes[0][2], a_boxes[0][3]],  [a_boxes[0][4], a_boxes[0][5]],
                      [a_boxes[0][6], a_boxes[0][7]]])
    M = cv2.getPerspectiveTransform(box, pts)
    res = cv2.warpPerspective(gaussian, M, (deta_y, deta_x))
    print(res)
